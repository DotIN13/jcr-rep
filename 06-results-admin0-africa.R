  ...
  for (j in viol) {
    ...
    for (k in fw) {
      ...
      print(xtable::xtable(metrics.temp.reg,
            ...
            hline.after = c(0, 0, 2, 4, 6, 8, 10, 12, 14),
            # floating.environment = "sidewaystable",
            file = paste0("tables/adm0_afr/", paste("metrics.adm0.afr", i, j, k, "tex", sep = ".")))

      ## this is to replace [ht] with [!htbp] to make four tables fit on one page in the appendix
      input_dir <- "tables/adm0_afr/"
      ...
      ggsave(
        filename = paste0("figures/metrics_plots/adm0_afr/", paste("mp.adm0.afr", i, j, k, "pdf", sep = ".")),
        metricsplot.reg.red,
        width = 12,
        height = 6,
        dpi = 300
...
for (j in viol) {
  ...
  for (k in fw) {
    ...
    ggsave(
      filename = paste0("figures/metrics_plots/adm0_afr/", paste("mp.adm0.afr", j, k, "pdf", sep = ".")),
      metricsplot, 
      width = 12,
      height = 6,
      dpi = 300
