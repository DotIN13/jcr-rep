  ...
  for (j in viol) {
    ...
    for (k in fw) {
      ...
      for (l in 1:length(col1)) {
        ...
        data.temp <- eval(readRDS(paste0("rds/predictions/cov/", paste(paste(tolower(col2[l]), "adm0_afr", i, j, tolower(col1[l]), k, sep = "_"), "rds", sep = "."))))
      ...
      for (l in 1:length(col4)) {
        ...
        model.temp <- eval(readRDS(paste0("rds/ml_models/cov/", paste(paste("rf_adm0_afr", i, j, tolower(col4[l]), k, sep = "_"), "rds", sep = "."))))
        ...
        if (col4[l] == "Gtw") {
          ...
          pdf(paste0("figures/pdp_plots/cov/adm0_afr/", paste("pdp.adm0.afr", i, j, ifelse(str_detect(col4[l], "_"), tolower(str_replace_all(col4[l], "_", ".")), tolower(col4[l])), k, "pdf", sep = ".")), width = 10, height = 6)
        ...
        ggsave(
          filename = paste0("figures/tuning_plots/cov/adm0_afr/", paste("tuning.adm0.afr", i, j, ifelse(str_detect(col4[l], "_"), tolower(str_replace_all(col4[l], "_", ".")), tolower(col4[l])), k, "pdf", sep = ".")),
          plot = toplot,
          width = 12,
          height = 6,
          dpi = 300
        ...
        ggsave(
          filename = paste0("figures/varimp_plots/cov/adm0_afr/", paste("varimp.adm0.afr", i, j, ifelse(str_detect(col4[l], "_"), tolower(str_replace_all(col4[l], "_", ".")), tolower(col4[l])), k, "pdf", sep = ".")),
          plot = toplot,
          width = 12,
          height = 6,
          dpi = 300
        ...
        #new_col_name <- paste(i, j, k, sep = "_")
        vimp.reg <- vimp %>%
          mutate(year = i,
                 type = j,
                 fw = k)
        ...
        print(xtable::xtable(vimp %>%
              ...
              file = paste0("tables/cov/adm0_afr/", paste("varimp.adm0.afr", i, j, ifelse(str_detect(col4[l], "_"), tolower(str_replace_all(col4[l], "_", ".")), tolower(col4[l])), k, "tex", sep = "."))
        )
      ...
      for (l in 1:length(col4)) {
        ...
        data.temp <- eval(readRDS(paste0("rds/predictions/cov/", paste(print(paste("test_adm0_afr", i, j, tolower(col4[l]), k, sep = "_")), "rds", sep = ".")))) %>%
          group_by(gwno) %>%
      ...
      ggsave(
        filename = paste0("figures/maps/cov/adm0_afr/", paste("map.adm0.afr", i, j, k, "png", sep = ".")),
        plot = toplot,
        width = 12,
        height = 6,
        dpi = 100
      ...
      for (m in unique(data.container$country_name)) {
        ...
        ggsave(
          filename = paste0("figures/line_plots/cov/adm0_afr/", paste("lp.adm0.afr", i, j, tolower(paste(m)), k, "pdf", sep = ".")),
          plot = toplot,
      ...
      print(xtable::xtable(metrics.temp.reg,
            ...
            hline.after = c(0, 0, 2, 4, 6, 8, 10, 12, 14),
            # floating.environment = "sidewaystable",
            file = paste0("tables/cov/adm0_afr/", paste("metrics.adm0.afr", i, j, k, "tex", sep = ".")))
      
      ## this is to replace [ht] with [!htbp] to make four tables fit on one page in the appendix
      input_dir <- "tables/cov/adm0_afr/"
      ...
      ggsave(
        filename = paste0("figures/metrics_plots/cov/adm0_afr/", paste("mp.adm0.afr", i, j, k, "pdf", sep = ".")),
        metricsplot.reg.red,
        width = 12,
        height = 6,
        dpi = 300
...
for (j in viol) {
  ...
  for (k in fw) {
    ...
    ggsave(
      filename = paste0("figures/metrics_plots/cov/adm0_afr/", paste("mp.adm0.afr", j, k, "pdf", sep = ".")),
      metricsplot, 
      width = 12,
      height = 6,
      dpi = 300
