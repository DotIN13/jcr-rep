  ...
  for (j in viol) {
    ...
    for (k in fw) {
      ...
      print(xtable::xtable(metrics.temp.reg,
            ...
            # floating.environment = "sidewaystable",
            file = paste0("tables/nonzero/adm0_glob/", paste("metrics.adm0.glob", i, j, k, "tex", sep = ".")))
      
      ## this is to replace [ht] with [!htbp] to make four tables fit on one page in the appendix
      input_dir <- "tables/nonzero/adm0_glob/"
      ...
      ggsave(
        filename = paste0("figures/metrics_plots/nonzero/adm0_glob/", paste("mp.adm0.glob", i, j, k, "pdf", sep = ".")),
        metricsplot.reg.red,
        width = 12,
        height = 6,
        dpi = 300
...
for (j in viol) {
  ...
  for (k in fw) {
    ...
    ggsave(
      filename = paste0("figures/metrics_plots/nonzero/adm0_glob/", paste("mp.adm0.glob", j, k, "pdf", sep = ".")),
      metricsplot, 
      width = 12,
      height = 6,
      dpi = 300
